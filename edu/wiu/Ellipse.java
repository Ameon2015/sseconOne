...
public class Ellipse extends AbstractDrawingObject{
   private double rMajor, rMinor;
   
   // constructor 
   public Ellipse( double x0, double y0, double rx, double ry, Color c0 ){
      super( x0, y0, c0);
      checkRadius( rx );
      checkRadius( ry );
      rMajor = rx;
   ...
   private void checkRadius( double r ){
      if( r <= 0 )
         throw new IllegalArgumentException( "Ellipse cannot have negative radius." );   
   }
   ...
   public  void draw(){
      ...
      // draw it
      StdDraw.filledEllipse( getX(), getY(), rMajor, rMajor );
